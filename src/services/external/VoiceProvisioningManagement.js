...
const MATCH_ALL_RULE_SCHEMA = Joi.object().keys({
  type: Joi.string().required(),
  description: Joi.string(),
});

const REGEX_RULE_SCHEMA = Joi.object().keys({
  type: Joi.string().required(),
  description: Joi.string(),
...
const SOURCE_NETWORK_RULE_SCHEMA = Joi.object().keys({
  type: Joi.string().required(),
  description: Joi.string(),
...
const MANIPULATION_RULE_SCHEMA = Joi.object().keys({
  description: Joi.string(),
...
const OFFNET_CALL_MANIPULATION_RULE_SCHEMA = Joi.object().keys({
  type: Joi.string().required(),
  description: Joi.string(),
...
const REGEX_FIELD_MANIPULATOR_RULE_SCHEMA = Joi.object().keys({
  type: Joi.string().required(),
  description: Joi.string(),
...
export class VoiceProvisioningManagement extends CpsRequest {
      ...
      trunks: Joi.array().items(
        Joi.object().keys({
          matchers: Joi.array().items(
            MATCH_ALL_RULE_SCHEMA,
            REGEX_RULE_SCHEMA,
            SOURCE_NETWORK_RULE_SCHEMA
          ).required(),
          manipulation_rules: Joi.array().items(
            MANIPULATION_RULE_SCHEMA,
            REGEX_FIELD_MANIPULATOR_RULE_SCHEMA,
            OFFNET_CALL_MANIPULATION_RULE_SCHEMA
          ),
  ...
  sipGatewayCreation({
    ...
    const rules = {
      identifier: Joi.string().required(),
      description: Joi.string(),
      manipulation_rules: Joi.array().items(MANIPULATION_RULE_SCHEMA),
      name: Joi.string(),
...
export default VoiceProvisioningManagement;
