...
const CAPABILITIES = ['im', 'im.im-to-sms', 'call', 'call.onnet', 'call.offnet', 'call.maaii-in', 'wallet', 'verification'];
...
export default function provisioningService(processManager, validator) {
  ...
  function onProcessComplete(provisionId, { taskResults, errors }) {
    if (errors) {
      Provisioning.findByIdAndUpdate(provisionId, { errors, taskResults, status: statusTypes.ERROR });
      return;
    }
  ...
  const schemaCreateProvision = Joi.object({
    ...
    capabilities: Joi.array().items(Joi.string().valid(CAPABILITIES)).unqiue().required(),
  ...
  const schemaGetProvisionings = Joi.object({
    ...
    pageSize: Joi.number().min(5).max(50).default(10),
  });

  async function getProvisionings(command) {
    const { page, pageSize, serviceType, companyCode, companyId } = validator.sanitize(command, schemaGetProvisionings);
